...
####### input pipeline 
def input_fn():
  train_files_names = os.listdir('/home/public/tfdata/')
  train_files = ['/home/public/tfdata/'+item for item in train_files_names]
...
def main():
  ...
  #else: strategy = tf.contrib.distribute.ParameterServerStrategy(num_gpus=NUM_GPUS)   # need tensorflow 1.11 version

  session_config = tf.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=False))

  config = tf.estimator.RunConfig(model_dir="/home/work/saved_models/",
#                                  train_distribute=strategy,save_checkpoints_steps=5000)                                 # distributed mode, fixed batch_size and fixed memory using
