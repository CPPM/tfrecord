...
model_dir = '/home/chaowang/workarea/Alluxio_test/dnn/saved_models_0/%s'%(args.timeline,)
if os.path.exists(model_dir):
    shutil.rmtree(model_dir)
os.makedirs(model_dir)
data_dir = '/home/tfdata_0/'
timeline_dir = '/home/chaowang/workarea/Alluxio_test/dnn/timeline/%s'%(args.timeline,)
...
def main():
  ...
  else: strategy = tf.contrib.distribute.MirroredStrategy(devices=['/device:GPU:2','/device:GPU:1'])
  #else: strategy = tf.contrib.distribute.MirroredStrategy(num_gpus=NUM_GPUS)
  #else: strategy = tf.contrib.distribute.ParameterServerStrategy(NUM_GPUS)   # need tensorflow 1.11 version

  session_config = tf.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=True))#, inter_op_parallelism_threads=2, intra_op_parallelism_threads=2)
  #session_config = None#tf.ConfigProto()
  #session_config.gpu_options.allow_growth = False

  config = tf.estimator.RunConfig(model_dir=model_dir,
                                  train_distribute=None, save_checkpoints_steps=5000, session_config = session_config)                                 # distributed mode, fixed batch_size and fixed memory using
