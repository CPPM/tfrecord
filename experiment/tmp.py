...
parser.add_argument('--type', '-y', default='ps')
...
def main():
  ...
  # else: 
  strategy = tf.contrib.distribute.ParameterServerStrategy(1)   # need tensorflow 1.11 version
  ...
  config = tf.estimator.RunConfig(model_dir=model_dir,train_distribute=strategy, save_checkpoints_steps=5000, session_config = session_config)                                 # distributed mode, fixed batch_size and fixed memory using
...
#                                  train_distribute=None,save_checkpoints_steps=5000)                                      # single mode, fixed batch_size and fixed memory using
  ...
  train_spec = tf.estimator.TrainSpec(input_fn=input_fn, hooks=[time_hist])
